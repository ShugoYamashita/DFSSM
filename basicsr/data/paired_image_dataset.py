...
from basicsr.data.custom_rain import paired_paths_from_folder_rainds, paired_paths_from_folder_DDN_Data, paired_paths_GT_Rain, paired_paths_GT_Rain_val
from basicsr.data.GTRain_aug import TrainDataGTRain_aug
...
class Dataset_PairedImage_RainDS(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        ...
        self.mean = opt['mean'] if 'mean' in opt else None
        self.std = opt['std'] if 'std' in opt else None
        self.dataset_type = opt['dataset_type'] if 'dataset_type' in opt else 'all'
        ...
        self.paths = paired_paths_from_folder_rainds(
            [self.lq_folder, self.gt_folder], ['lq', 'gt'],
            self.filename_tmpl, self.dataset_type)
...
class Dataset_PairedImage_DDN_Data(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        ...
        self.paths = paired_paths_from_folder_DDN_Data(
            [self.lq_folder, self.gt_folder], ['lq', 'gt'])
...
class Dataset_PairedImage_GTRain(data.Dataset):
    """Paired image dataset for GTRain
    """
    ...
    def __init__(self, opt):
        ...
        self.paths = paired_paths_GT_Rain(
            [self.lq_folder, self.gt_folder], ['lq', 'gt'])
...
class Dataset_PairedImage_GTRain_Val(data.Dataset):
    """Paired image dataset for GTRain
    """
    ...
    def __init__(self, opt):
        ...
        self.paths = paired_paths_GT_Rain_val(
            [self.lq_folder, self.gt_folder], ['lq', 'gt'])
...
class Dataset_PairedImage_GTRain_custom_aug(data.Dataset):
    """Paired image dataset for GTRain
    """
    ...
    def __getitem__(self, index):
        ...
        # augmentation for training
        if self.opt['phase'] == 'train':
            TrainDataGTRain_aug(tar_img=img_gt, inp_img=img_lq, use_normalize=False)
